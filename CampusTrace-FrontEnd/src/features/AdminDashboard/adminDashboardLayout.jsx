    ...
    <RouterNavLink
      ...
      className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium
        ${
          isActive
            ? "bg-red-500/20 text-red-500 border-l-4 border-red-500"
            : "text-neutral-600 dark:text-zinc-400 hover:bg-neutral-200 dark:hover:bg-zinc-800 hover:text-neutral-900 dark:hover:text-white border-l-4 border-transparent"
        }
        ${!isOpen ? "justify-center" : ""}
      `}
    >
...
export default function AdminDashboardLayout({ children, user }) {
  ...
  return (
    <div className="h-screen flex flex-col bg-neutral-100 dark:bg-gradient-to-br dark:from-zinc-950 dark:to-black text-neutral-900 dark:text-zinc-300">
      {mobileMenu && (
        <div
          className="fixed inset-0 bg-black/50 z-40 md:hidden transition-opacity duration-200"
          onClick={() => setMobileMenu(false)}
      ...
      <header className="h-16 px-4 lg:px-8 bg-white/70 dark:bg-black/70 backdrop-blur-md border-b border-neutral-200 dark:border-zinc-800 flex items-center justify-between shadow-lg z-30 flex-shrink-0">
        <div className="flex items-center gap-4">
          <button
            onClick={() => setMobileMenu(!mobileMenu)}
            className="md:hidden p-2 text-neutral-600 dark:text-zinc-400 hover:bg-neutral-200 dark:hover:bg-zinc-800 hover:text-neutral-900 dark:hover:text-white rounded-md transition-colors"
            aria-label="Toggle menu"
          >
          ...
          <button
            onClick={toggleSidebar}
            className="hidden md:block p-2 text-neutral-600 dark:text-zinc-400 hover:bg-neutral-200 dark:hover:bg-zinc-800 hover:text-neutral-900 dark:hover:text-white rounded-md transition-colors"
            aria-label="Toggle sidebar"
          >
            <Menu className="w-5 h-5" />
          </button>
          <h1 className="text-xl font-semibold text-neutral-900 dark:text-white">Admin Panel</h1>
        </div>

        <div className="flex items-center gap-3">
          <button className="p-2 text-neutral-600 dark:text-zinc-400 hover:bg-neutral-200 dark:hover:bg-zinc-800 hover:text-neutral-900 dark:hover:text-white rounded-md transition-colors relative">
            <Bell className="w-5 h-5" />
            <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full"></span>
          </button>
          <div className="flex items-center gap-2">
            <img
              src={`https://ui-avatars.com/api/?name=${getInitial()}&background=ef4444&color=ffffff`}
              alt="Admin"
              className="w-8 h-8 rounded-full border-2 border-neutral-300 dark:border-zinc-700"
            />
            {user?.email && (
              <span className="hidden lg:block text-sm text-neutral-600 dark:text-zinc-400">
                {user.email}
              </span>
            )}
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside
          className={`
            fixed md:relative inset-y-0 left-0 z-50 
            bg-white/95 dark:bg-zinc-900/95 md:bg-white md:dark:bg-black backdrop-blur-md md:backdrop-blur-none
            border-r border-neutral-200 dark:border-zinc-800
            flex flex-col transition-all duration-300 
            p-4
            ${mobileMenu ? "translate-x-0 w-64" : "-translate-x-full"}
        ...
        >
          ...
          <div className="mt-auto">
            <button
              onClick={handleLogout}
              disabled={isLoggingOut}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-neutral-600 dark:text-zinc-400 hover:bg-red-500/10 dark:hover:bg-zinc-800 hover:text-red-500 dark:hover:text-red-500 transition-colors disabled:opacity-50
