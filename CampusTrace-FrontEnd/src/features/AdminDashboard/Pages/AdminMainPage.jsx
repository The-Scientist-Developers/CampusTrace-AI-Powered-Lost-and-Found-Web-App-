...
const StatCardSkeleton = () => (
  <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-xl shadow-sm flex items-center gap-4">
    ...
    <div>
      ...
      <Skeleton height={20} width={120} className="mt-1" /> {/* Title */}
    </div>
  </div>
);
...
const ActivityItemSkeleton = () => (
  <li className="flex gap-4 pb-4">
    ...
    <div className="flex-grow">
      ...
      <Skeleton height={16} width="50%" className="mt-1" /> {/* Time */}
    </div>
  </li>
);
    ...
    const fetchAdminData = async () => {
      try {
        ...
        const [
          ...
          supabase
            .from("items")
            .select("id, title, created_at") // Ensure 'id' is selected for the key prop
            .order("created_at", { ascending: false })
            .limit(5)
            .eq("university_id", adminUniversityId),
        ]);
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              <XAxis
                dataKey="name"
                stroke="currentColor"
                className="text-neutral-500 dark:text-neutral-400" // Added dark mode color
                fontSize={12}
                tickLine={false}
                axisLine={false}
              />
              ...
              <Tooltip
                cursor={{ fill: "rgba(156, 163, 175, 0.1)" }}
                contentStyle={{
                  backgroundColor: "rgba(255, 255, 255, 0.9)", // Adjusted background
                  border: "1px solid #e5e7eb",
                  borderRadius: "0.5rem",
                  boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)", // Added shadow
                }}
                itemStyle={{ color: "#374151" }} // Ensure item text color
                labelStyle={{ color: "#1f2937", fontWeight: "bold" }} // Ensure label color and weight
              />
              <Bar dataKey="posts" radius={[4, 4, 0, 0]}>
                {weeklyPosts.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={"#6366f1"} /> // Using a consistent primary color
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-xl shadow-sm">
          ...
          {recentActivity.length > 0 ? (
            <ul className="space-y-4">
              {recentActivity.map((activity) => (
                <ActivityItem
                  key={activity.id} // Use the unique ID from the data
                  text={`New post: "${activity.title}"`}
                  time={new Date(activity.created_at).toLocaleString()}
                  icon={MessageSquare}
                />
