...
const UserCard = ({ user, changeRole, toggleBan }) => (
  <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-lg p-4 space-y-4">
    <div className="flex items-center gap-4">
      <img
        src={`https://ui-avatars.com/api/?name=${
          user.full_name || user.email
      ...
      />
      <div className="flex-1">
        <p className="font-bold text-neutral-900 dark:text-white">{user.full_name || "No Name"}</p>
    ...
    <div className="grid grid-cols-2 gap-4 text-sm border-y border-neutral-200 dark:border-neutral-800 py-3">
      <div>
        <p className="text-neutral-600 dark:text-neutral-500">Role</p>
        <p className="font-semibold text-neutral-900 dark:text-white">{user.role}</p>
      </div>
      <div>
        <p className="text-neutral-600 dark:text-neutral-500">Status</p>
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${
            user.is_banned
              ? "bg-red-500/20 text-red-400"
    ...
    <div className="flex gap-2">
      <select
        value={user.role || "member"}
        onChange={(e) => changeRole(user.id, e.target.value)}
        className="flex-1 px-3 py-2 bg-neutral-100 dark:bg-zinc-800 border border-neutral-300 dark:border-zinc-700 rounded-md text-neutral-900 dark:text-white text-sm"
      >
        <option value="member">Member</option>
        <option value="moderator">Moderator</option>
        <option value="admin">Admin</option>
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-600 dark:text-zinc-400 mt-1">
            Manage users and roles for your university
          </p>
        </div>
        <div className="relative w-full md:w-64">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-neutral-600 dark:text-zinc-500" />
          <input
            type="text"
            placeholder="Search by name or email..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-4 py-2 bg-neutral-100 dark:bg-zinc-800 border border-neutral-300 dark:border-zinc-700 rounded-lg text-neutral-900 dark:text-white placeholder-neutral-600 dark:placeholder-zinc-500"
          />
        </div>
      </div>
      ...
      <div className="hidden md:block bg-white dark:bg-zinc-900 rounded-lg border border-neutral-200 dark:border-zinc-800 overflow-hidden">
        <table className="w-full">
          <thead className="bg-neutral-100 dark:bg-zinc-800/50 border-b border-neutral-200 dark:border-zinc-800">
            <tr>
              <th className="text-left p-4 text-neutral-600 dark:text-zinc-400 font-medium">User</th>
              <th className="text-left p-4 text-neutral-600 dark:text-zinc-400 font-medium">Role</th>
              <th className="text-left p-4 text-neutral-600 dark:text-zinc-400 font-medium">Status</th>
              <th className="text-center p-4 text-neutral-600 dark:text-zinc-400 font-medium">Items Posted</th>
              <th className="text-left p-4 text-neutral-600 dark:text-zinc-400 font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.length === 0 ? (
              <tr>
                <td colSpan="5" className="text-center p-8 text-neutral-600 dark:text-zinc-500">
                  No users found.
                </td>
              </tr>
            ) : (
              users.map((user) => (
                <tr
                  key={user.id}
                  className="border-b border-neutral-200 dark:border-zinc-800 last:border-b-0 hover:bg-neutral-100 dark:hover:bg-zinc-800/30"
                >
                  <td className="p-4 align-top">
                    <div>
                      ...
                      <div className="text-neutral-600 dark:text-zinc-400 text-sm">{user.email}</div>
                    </div>
                  </td>
                  <td className="p-4 align-top">
                    <select
                      value={user.role || "member"}
                      onChange={(e) => changeRole(user.id, e.target.value)}
                      className="px-3 py-1 bg-neutral-100 dark:bg-zinc-800 border border-neutral-300 dark:border-zinc-700 rounded text-neutral-900 dark:text-white text-sm"
                    >
                      <option value="member">Member</option>
                      <option value="moderator">Moderator</option>
                      <option value="admin">Admin</option>
                  ...
                  <td className="p-4 text-neutral-900 dark:text-white text-center align-top">
                    {user.items?.[0]?.count || 0}
                  </td>
                  <td className="p-4 align-top">
                    <button
      ...
      <div className="block md:hidden space-y-4">
        {users.length === 0 ? (
          <div className="text-center p-8 text-neutral-600 dark:text-zinc-500">No users found.</div>
        ) : (
          users.map((user) => (
            <UserCard
              key={user.id}
