...
const StatCard = ({ label, value, icon: Icon }) => (
  <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-xl shadow-lg flex items-center gap-4">
    <Icon className="w-8 h-8 text-red" />
    <div>
...
export default function UserProfilePage({ user }) {
  ...
  if (loading) {
    return (
      <div className="p-8 text-center text-neutral-600 dark:text-zinc-400">Loading profile...</div>
    );
  }
  if (error) {
    return <div className="p-8 text-center text-red">{error}</div>;
  ...
  if (!profile) {
    return (
      <div className="p-8 text-center text-neutral-600 dark:text-zinc-400">Profile not found.</div>
    );
  }

  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 space-y-8 animate-fadeIn bg-white dark:bg-transparent">
      {/* Profile Header */}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="relative">
            <img
              ...
              alt="Avatar"
              className="w-32 h-32 rounded-full border-4 border-neutral-300 dark:border-neutral-700 object-cover"
            />
            {isEditing && (
              <label
                htmlFor="avatar-upload"
          ...
          <div className="flex-1 text-center sm:text-left">
            {isEditing ? (
              <input
                ...
                value={fullName}
                onChange={(e) => setFullName(e.target.value)}
                className="text-3xl font-bold text-neutral-900 dark:text-white bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-1 mb-2 w-full"
              />
            ) : (
          ...
          {isEditing ? (
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setIsEditing(false)}
                className="p-2 text-neutral-600 dark:text-zinc-400 hover:bg-neutral-200 dark:hover:bg-zinc-700 rounded-md"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <button
              onClick={() => setIsEditing(true)}
              className="px-4 py-2 bg-neutral-200 dark:bg-zinc-700 text-neutral-900 dark:text-white font-semibold text-sm rounded-md hover:bg-neutral-300 dark:hover:bg-zinc-600 transition flex items-center gap-2"
            >
              <Edit className="w-4 h-4" />
              Edit Profile
            </button>
      ...
      <div>
        ...
        {posts.length > 0 ? (
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-lg p-4 divide-y divide-neutral-200 dark:divide-neutral-800">
            {posts.slice(0, 5).map((post) => (
              <div key={post.id} className="flex items-center gap-4 py-3">
                <div className="w-12 h-12 bg-neutral-200 dark:bg-neutral-800 rounded-md flex-shrink-0 flex items-center justify-center">
                  {post.image_url ? (
                    <img
                      src={post.image_url}
                      alt={post.item_name}
