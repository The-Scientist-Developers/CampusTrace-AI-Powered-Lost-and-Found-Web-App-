...
export default function MyPostsPage({ user }) {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[300px] p-8 text-neutral-600 dark:text-zinc-400">
        <Loader2 className="w-8 h-8 animate-spin mr-3" />
        Loading your posts...
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex border-b border-neutral-300 dark:border-zinc-700 mb-6">
        {["active", "pending", "resolved"].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-4 sm:px-6 py-3 text-base sm:text-lg font-medium transition-colors duration-200 
              ${
                activeTab === tab
                  ? "text-red border-b-2 border-red"
                  : "text-neutral-600 dark:text-zinc-400 hover:text-neutral-900 dark:hover:text-zinc-200 hover:border-neutral-500 dark:hover:border-zinc-500 border-b-2 border-transparent"
              }`}
          >
            {tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
      ...
      {posts.length === 0 ? (
        <div className="text-center p-12 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-lg text-neutral-600 dark:text-zinc-400">
          <p className="text-lg">No {activeTab} posts found.</p>
          <p className="mt-2 text-sm">
            When you create a post, it will appear here.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {posts.map((post) => (
            <div
              key={post.id}
              className="group relative bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl shadow-lg overflow-hidden transition-transform hover:-translate-y-1 flex flex-col h-full"
            >
              <button
                onClick={() => handleDeletePost(post.id)}
                className="absolute top-2 right-2 z-10 p-1.5 bg-black/50 text-neutral-700 dark:text-zinc-300 rounded-full
                          opacity-0 group-hover:opacity-100 transition-opacity
                          hover:bg-red/80 hover:text-white"
                title="Delete Post"
              >
              ...
              <div className="w-full h-48 flex items-center justify-center bg-neutral-100 dark:bg-zinc-800 p-2">
                {post.image_url ? (
                  ...
                  <ImageIcon className="w-16 h-16 text-neutral-600 dark:text-zinc-500" />
                )}
              </div>
              <div className="p-4 flex flex-col flex-grow">
                ...
                <p className="text-sm text-neutral-600 dark:text-zinc-400 mb-2 line-clamp-2 flex-grow">
                  {post.description}
                </p>
                <div className="text-xs text-neutral-600 dark:text-zinc-500 space-y-1 mb-4 border-t border-neutral-200 dark:border-neutral-800 pt-3 mt-auto">
                  <p>
                    <strong>Type:</strong> {post.category}
                  </p>
                  <p>
