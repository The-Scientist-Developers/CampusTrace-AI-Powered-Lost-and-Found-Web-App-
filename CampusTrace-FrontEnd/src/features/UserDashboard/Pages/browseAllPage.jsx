...
//           <div className="w-full h-64 flex items-center justify-center bg-neutral-100 dark:bg-zinc-800 p-2 rounded-lg">
//             {item.image_url ? (
//               <img
//                 src={item.image_url}
//                 alt={item.title}
...
// --- (No changes to getAccessToken or apiClient) ---
async function getAccessToken() {
  const { data } = await supabase.auth.getSession();
  return data?.session?.access_token || null;
}
...
// --- (No changes to ItemDetailsModal) ---
const ItemDetailsModal = ({ item, onClose, onClaim, user }) => {
  if (!item) return null;
  const posterName =
    item.profiles?.full_name ||
...
// --- 2. ADD SKELETON CARD COMPONENT (before main component) ---
  ...
  const fetchPosts = useCallback(
    async (isSearchReset = false) => {
      ...
      try {
        ...
        query = query.range(from, to).order("created_at", { ascending: isAscending });
        const { data, error, count } = await query;
        if (error) throw error;
        setPosts(data || []);
        setTotalPosts(count || 0);
