  ...
  return (
    <RouterNavLink
      ...
      className={({ isActive }) => `
                ...
                ${
                  isActive
                    ? "bg-gradient-to-r from-red-600/30 to-red-700/20 text-red-300 dark:text-red-300 border-l-4 border-red-500 shadow-lg shadow-red-500/10"
                    : "text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800/60 hover:text-zinc-900 dark:hover:text-white border-l-4 border-transparent"
                }
                ${!isOpen ? "justify-center" : ""}
                active:scale-95
            `}
        ...
        <>
          ...
          {isOpen && (
            <>
              ...
              {item.badge && (
                <span className="px-2 py-0.5 text-xs bg-red-600 text-gray rounded-full font-bold min-w-[20px] text-center shadow-md">
                  {item.badge}
                </span>
              )}
            </>
...
export default function DashboardLayout({ children, user }) {
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-zinc-50 to-zinc-100 dark:from-zinc-950 dark:to-black text-zinc-800 dark:text-zinc-300 overflow-hidden">
      {mobileMenu && (
        <div
          className="fixed inset-0 bg-black/70 z-40 md:hidden"
          onClick={() => setMobileMenu(false)}
      ...
      <header className="h-14 sm:h-16 px-3 sm:px-4 lg:px-6 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-b border-zinc-200 dark:border-zinc-800/60 flex items-center justify-between shadow-2xl z-30 flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            onClick={() => setMobileMenu(!mobileMenu)}
            className="md:hidden p-1.5 sm:p-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white rounded-lg"
          >
            {mobileMenu ? (
              <X className="w-5 h-5 sm:w-6 sm:h-6" />
            ) : (
          ...
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="hidden md:block p-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white rounded-lg"
          >
            <Menu className="w-5 h-5" />
          </button>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3">
          <button
            onClick={() => navigate("/dashboard/notifications")}
            className="p-1.5 sm:p-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white rounded-lg relative"
          >
            <Bell className="w-4 h-4 sm:w-5 sm:h-5" />
            {notificationCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1">
          ...
          <button
            onClick={() => navigate("/dashboard/post-new")}
            className="px-2.5 py-1.5 sm:px-4 sm:py-2 bg-red-600 text-gray font-semibold text-xs sm:text-sm rounded-lg hover:bg-red-700 flex items-center gap-1.5 sm:gap-2"
          >
            <Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="hidden xs:inline sm:inline">Post</span>
            <span className="hidden sm:inline">New Item</span>
          ...
          <div className="hidden sm:flex items-center gap-2">
            <img
              src={avatarUrl}
              alt="User"
              className="w-8 h-8 rounded-full border-2 border-zinc-300 dark:border-zinc-700 hover:border-red-500 cursor-pointer"
              onClick={() => navigate("/dashboard/profile")}
            />
            <span className="hidden lg:block text-sm text-zinc-600 dark:text-zinc-400 max-w-[120px] xl:max-w-[180px] truncate">
              {displayName}
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside
          className={`fixed md:relative inset-y-0 left-0 z-50 bg-white/98 dark:bg-zinc-900/98 md:bg-white/95 md:dark:bg-black/95 backdrop-blur-xl border-r border-zinc-200 dark:border-zinc-800/60 flex flex-col transition-all duration-300 ease-in-out top-14 sm:top-16 md:top-0 ${
            mobileMenu
              ? "translate-x-0 w-72 shadow-2xl"
              : "-translate-x-full md:translate-x-0"
          } ${
        ...
        >
          {(isSidebarOpen || mobileMenu) && (
            <div className="p-3 sm:p-4 flex-shrink-0">
              <div className="p-3 sm:p-4 bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-900 rounded-xl border border-zinc-300 dark:border-zinc-700/50 shadow-lg">
                ...
                <p className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-white mb-1">
                  {itemsPostedCount}
                </p>
                <p className="text-xs text-zinc-500 dark:text-zinc-500">Items Posted</p>
              </div>
            </div>
          )}
          <nav className="flex-1 p-2 sm:p-3 space-y-1 overflow-y-auto">
            ...
            <div className="my-2 sm:my-3 border-t border-zinc-200 dark:border-zinc-800/60" />
            {computedMenuItems.map((item, i) => (
              <NavLink
                key={`menu-${i}`}
                item={item}
          ...
          <div className="p-2 sm:p-3 border-t border-zinc-200 dark:border-zinc-800/60 flex-shrink-0 space-y-2 sm:space-y-3 bg-zinc-50 dark:bg-black/20">
            {(isSidebarOpen || mobileMenu) && user && (
              <div className="px-3 py-2.5 bg-zinc-100 dark:bg-zinc-800/60 rounded-lg border border-zinc-300 dark:border-zinc-700/50">
                <p className="text-[10px] text-zinc-600 dark:text-zinc-500 font-medium uppercase tracking-wide">
                  Logged in as
                </p>
                <p className="text-sm text-zinc-900 dark:text-white truncate font-medium mt-0.5">
                  {user.email}
                </p>
              </div>
            )}
            <button
              onClick={handleLogout}
              disabled={isLoggingOut}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-600 dark:text-zinc-400 hover:bg-red-100 dark:hover:bg-red-500/20 hover:text-red-600 dark:hover:text-red-400 transition-all duration-200 disabled:opacity-50 ${
                !isSidebarOpen && !mobileMenu ? "justify-center" : ""
              }`}
            >
              <LogOut className="w-5 h-5 flex-shrink-0" />
        ...
        <main className="flex-1 overflow-y-auto bg-gradient-to-b from-zinc-50 to-zinc-100 dark:from-zinc-900/30 dark:to-black/50">
          <div className="p-3 sm:p-4 md:p-6 lg:p-8 min-h-full">{children}</div>
        </main>
      </div>
    </div>
