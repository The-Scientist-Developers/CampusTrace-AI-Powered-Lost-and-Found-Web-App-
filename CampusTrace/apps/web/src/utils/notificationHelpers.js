 ...
 */
export const sendNotification = async (recipientId, message, linkTo = null, universityId = null) => {
  try {
    console.log("📬 Inserting notification:", { recipientId, message, linkTo, universityId }); // DEBUG
    ...
    if (!finalUniversityId) {
      ...
      console.log("🏫 Fetched university_id:", finalUniversityId); // DEBUG
    }
 ...
 */
export const notifyPostStatusUpdate = async (userId, postTitle, status, postId) => {
  console.log("🚀 notifyPostStatusUpdate called with:", { userId, postTitle, status, postId }); // DEBUG
  ...
  console.log("📧 Preparing notification:", { userId, message, linkTo }); // DEBUG
  
  const result = await sendNotification(userId, message, linkTo);
  
  console.log("✉️ Notification send result:", result); // DEBUG
 ...
 */
export const notifyClaimStatusUpdate = async (userId, itemTitle, status, claimId) => {
  const message =
    status === "approved"
