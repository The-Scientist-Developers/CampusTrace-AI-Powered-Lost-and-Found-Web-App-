...
# @onboarding_router.post("/submit-manual-verification")
...
# ):
#     """
#     Handles the submission of a user's university ID for manual verification.
#     """
#     try:
#         # 1. Upload the ID to a secure storage bucket
#         file_bytes = await id_file.read()
#         file_suffix = Path(id_file.filename or "").suffix
#         file_path = f"manual_verifications/{user_id}/{uuid4().hex}{file_suffix}"
...
#         # 2. Create a record in a 'manual_verifications' table
...
#         insert_response = supabase.table("manual_verifications").insert(verification_data).execute()
#         if not insert_response.data:
#             raise Exception("Failed to create verification record.")

#         # 3. Notify admins of the selected university
#         admins_res = supabase.table("profiles").select("id, full_name, email").eq("university_id", university_id).eq("role", "admin").execute()
        
#         user_profile_res = supabase.table("profiles").select("full_name, email").eq("id", user_id).single().execute()
#         user_name = user_profile_res.data.get('full_name') or user_profile_res.data.get('email')

#         if admins_res.data:
#             message = f"New manual verification request from {user_name} is awaiting your review."
#             for admin in admins_res.data:
#                 create_notification(
#                     recipient_id=admin['id'],
#                     university_id=university_id,
#                     message=message,
#                     link_to="/admin/user-management", # Or a dedicated verification page
#                     type='verification'
#                 )
...
async def signup_manual(
    ...
    try:
        ...
        sign_up_res = supabase.auth.sign_up({
            "email": email,
            "password": password,
            "options": { "email_redirect_to": settings.EMAIL_CONFIRM_REDIRECT }
        })
        if not sign_up_res.user:
            raise Exception("Failed to create user in Auth.")
...
async def respond_to_verification(
    ...
    """
    try:
        # Verify admin has permission for this request (optional but good practice)
        verification_res = supabase.table("user_verifications").select("university_id").eq("id", verification_id).single().execute()
        ...
        if action.approve:
            update_res = supabase.table("profiles").update({
                "university_id": verification_res.data['university_id'],
                "is_verified": True # Assuming you have an 'is_verified' column
            }).eq("id", action.user_id).execute()
            ...
            create_notification(
                recipient_id=action.user_id,
                university_id=verification_res.data['university_id'],
                message="Your manual verification request was not approved. Please ensure your ID is clear and valid.",
                link_to="/manual-verification",
